...
TASA = 44100
VENTANA = 2048
SALTO = TASA // 50              # 50 actualizaciones por segundo
    ...
    while True:
        ...
        try:
            while True:
                ...
                buffer = np.roll(buffer, -len(muestras))
                buffer[-len(muestras):] = muestras
                if not clientes:
                    continue

                espectro = np.abs(np.fft.rfft(buffer * ventana))
                bandas = np.array([espectro[idx[i]:idx[i + 1]].max() for i in range(BARRAS)])
                bandas = np.log1p(bandas * realce)

                # Ganancia automática: el techo sigue al máximo y baja despacio
                techo = max(techo * 0.998, float(bandas.max()), 3.0)
                valores = np.clip(bandas / techo, 0, 1) ** 1.6

                # Suben rápido y caen suave
                previo = np.where(valores > previo,
                                  previo + (valores - previo) * 0.7,
                                  np.maximum(valores, previo * 0.86)).astype(np.float32)

                mensaje = marco_texto(json.dumps([round(float(v), 3) for v in previo], separators=(",", ":")))
